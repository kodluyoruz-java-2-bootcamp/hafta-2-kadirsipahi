...
public class Main {
    ...
    private static void testComputer1()
    {
        ...
        {
            throw new RuntimeException(String.format("Fiyat hesaplama metodunuz yanlýþ! Beklenen: %.2f Çýkan: %.2f", estimatedTotalPrice, totalPrice));
        }
        final int totalPower = computer.getTotalPower();
        final int estimatedTotalPower = 550;
        if (estimatedTotalPower != totalPower)
        {
            throw new RuntimeException(String.format("Güç tüketimi hesaplama metodunuz yanlýþ! Beklenen: %d Çýkan: %d", estimatedTotalPower, totalPower));
        }
    }
